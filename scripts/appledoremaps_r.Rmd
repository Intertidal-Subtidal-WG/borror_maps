...
keen_data <- read.csv(here('data', 'transects', 'keen_all_methods_site_merged.csv'))
```
...
keen data
```{r}
#keep it to this for now in case there's other stuff
keen_data2 <- keen_data %>%
  select(NETWORK.x, YEAR.x, SITE.x, TRANSECT.x, PHYLUM, CLASS, GENUS, SPECIES,
         START_LATITUDE, START_LONGITUDE, END_LATITUDE, END_LONGITUDE) %>%
  na.omit(SPECIES)

#rename

keen_data_merge <- keen_data2 %>%
  select(TRANSECT.x, YEAR.x, SPECIES, START_LATITUDE, START_LONGITUDE) %>%
  rename(Organism = SPECIES,
         Transect = TRANSECT.x,
         dec_lat = START_LATITUDE,
         dec_long = START_LONGITUDE, 
         Year = YEAR.x)

inter_merge <- intertidal_combine %>%
  select(Transect, Year, Organism, dec_lat, dec_long) %>%
  mutate(Transect = as.character(Transect))


merged_inter_sub <- full_join(keen_data_merge, inter_merge)
```


```{r}
#sp records for specific years
#multiple species by this method can result in the species being different colors per map
#sp_list <- c('Strongylocentrotus droebachiensis', 'Cancer borealis', 
#             'Cancer irroratus', "Semibalanus balanoides")
...
merged_all_sub <- merged_inter_sub %>% subset(Organism %in% sp_list)
...
y1982 <- merged_all_sub %>% subset(Year == 1982)
y1983 <- merged_all_sub %>% subset(Year == 1983)
y1984 <- merged_all_sub %>% subset(Year == 1984)
y1985 <- merged_all_sub %>% subset(Year == 1985)
y1986 <- merged_all_sub %>% subset(Year == 1986)
y1987 <- merged_all_sub %>% subset(Year == 1987)
y1988 <- merged_all_sub %>% subset(Year == 1988)
y1989 <- merged_all_sub %>% subset(Year == 1989)
y1990 <- merged_all_sub %>% subset(Year == 1990)
y1991 <- merged_all_sub %>% subset(Year == 1991)
y1992 <- merged_all_sub %>% subset(Year == 1992)
...
ggsave(here("figures", "2021115_test_2.png"), plot = sp_plots_m, device = "png",
...
ggsave(here("figures", "2021115_test2_2.png"), plot = sp_plots_s, device = "png",
       width = 7, height = 8, units = 'in')
